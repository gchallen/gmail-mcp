...
async function main() {
    ...
    switch (cmd) {
        ...
        case 'status': {
            ...
            console.log(`OAuth client:     ${fs.existsSync(credentialsPath()) ? credentialsPath() : 'MISSING (credentials.json)'}`);
            console.log(`Token:            ${fs.existsSync(tokenPath()) ? tokenPath() : 'MISSING (run gmail-mcp auth)'}`);
            const v = await verifyToken();
            console.log(v.ok ? `Account:          ${v.email}` : `Account:          not available (${v.error})`);
            if (v.ok) {
        ...
        case 'call': {
            ...
            if (!t)
                throw new Error(`Unknown tool "${name}". Run \`gmail-mcp tools\`.`);
