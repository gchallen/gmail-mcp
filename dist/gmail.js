...
export class GmailService {
    ...
    // ---- reading --------------------------------------------------------------
    async getMessage(id, format = 'plain_text') {
    ...
    async getMessages(ids, format) {
        return mapLimit(ids, CONCURRENCY, (id) => this.getMessage(id, format));
    }
    async getThread(id, format = 'plain_text') {
    ...
    async searchThreads(opts) {
        ...
        let q = opts.query ?? '';
        if (!/\b(in|is|label):drafts?\b/i.test(q))
            q = `${q} -in:draft`.trim();
    ...
    async fetchAttachment(messageId, att) {
        if (!att.attachmentId) {
            ...
            const find = (p) => {
                if (!p)
                    return undefined;
                if (p.filename === att.filename && p.body?.data)
                    return p.body.data;
    ...
    async saveAttachments(opts) {
        ...
        if (opts.attachmentId)
            wanted = all.filter((a) => a.attachmentId === opts.attachmentId);
        else {
            if (!opts.includeInline)
                wanted = wanted.filter((a) => !a.inline);
    ...
    async loadMessageAttachments(messageId, includeInline = true) {
        const atts = await this.listAttachments(messageId);
        return mapLimit(atts.filter((a) => includeInline || !a.inline), CONCURRENCY, async (a) => ({
            filename: a.filename,
            mimeType: a.mimeType,
    ...
    async replyRecipients(original, replyAll) {
    ...
    async upload(kind, raw, threadId, draftId) {
    ...
    async createDraft(input, mode = 'new') {
        const { raw, threadId, summary } = await this.compose(input, mode);
        const draft = (await this.upload('draft', raw, threadId));
        return {
            draftId: draft.id,
    ...
    async sendNew(input, mode = 'new') {
        const { raw, threadId, summary } = await this.compose(input, mode);
        const sent = (await this.upload('send', raw, threadId));
    ...
    async forward(opts) {
        const originalAttachments = opts.includeAttachments === false
            ? []
    ...
    async getDraft(draftId, format = 'plain_text') {
    ...
    async updateDraft(opts) {
        ...
        let attachments = [];
        if (opts.attachments)
            attachments = opts.attachments.map(loadAttachment);
        ...
        const sig = opts.signature === false ? undefined : bodyChanged ? await this.getSignature() : undefined;
        let text;
        let html;
        if (bodyChanged) {
        ...
        const raw = buildMime(out);
        const draft = (await this.upload('draft', raw, msg.threadId ?? undefined, opts.draftId));
        return {
            draftId: draft.id,
            messageId: draft.message?.id,
    ...
    async listLabels() {
        ...
        labels.sort((a, b) => a.type === b.type ? (a.name ?? '').localeCompare(b.name ?? '') : a.type === 'system' ? -1 : 1);
        return labels;
    }
    ...
    spamMessage = (id) => this.modifyMessage(id, ['SPAM'], ['INBOX']);
    unspamMessage = (id) => this.modifyMessage(id, ['INBOX'], ['SPAM']);
    spamThread = (id) => this.modifyThread(id, ['SPAM'], ['INBOX']);
    unspamThread = (id) => this.modifyThread(id, ['INBOX'], ['SPAM']);
}
export const displayName = (addr) => decodeHeaderValue(addr)
    .replace(/<[^>]+>/, '')
    .trim()
