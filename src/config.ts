...
export interface UserConfig {
  /** Plain-text signature appended to outgoing mail. */
  signature?: string;
  /** HTML signature; derived from `signature` when absent. */
  signatureHtml?: string;
  /** Set false to never append a signature. */
...
export function loadUserConfig(): UserConfig {
  ...
  if (process.env.GMAIL_SIGNATURE) cfg.signature = process.env.GMAIL_SIGNATURE;
  if (process.env.GMAIL_SIGNATURE_HTML) cfg.signatureHtml = process.env.GMAIL_SIGNATURE_HTML;
