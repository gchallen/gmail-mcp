...
/** An attachment given either as a local path or as inline content. */
export interface AttachmentSpec {
  /** Absolute path to a local file. */
  path?: string;
  /** Base64 content, when the file is not on disk. */
  content?: string;
  /** Overrides the basename of `path`; required with `content`. */
  filename?: string;
  mimeType?: string;
  /** Inline (Content-ID) image for use as <img src="cid:..."> in htmlBody. */
  inline?: boolean;
  /** Content-ID to use; defaults to the filename. */
  contentId?: string;
}
...
export function loadAttachment(spec: AttachmentSpec | string): LoadedAttachment {
  const s: AttachmentSpec = typeof spec === 'string' ? { path: spec } : spec;
  let data: Buffer;
  let filename: string;
  if (s.path) {
...
export function encodeAddress(addr: string): string {
  ...
  if (!name) return `<${m[2]}>`;
  if (needsEncoding(name)) return `${encodeHeaderWord(name)} <${m[2]}>`;
  return /[^\w\s.'-]/.test(name) ? `"${name.replace(/"/g, '\\"')}" <${m[2]}>` : `${name} <${m[2]}>`;
}

function encodeFilenameParam(filename: string): string {
  if (!needsEncoding(filename) && !/["\\]/.test(filename)) return `filename="${filename}"`;
...
const boundary = (tag: string) =>
  `----=_${tag}_${Date.now().toString(36)}_${(counter++).toString(36)}`;
...
export function buildMime(m: OutgoingMessage): string {
  const H: string[] = [];
  if (m.from) H.push(`From: ${encodeAddress(m.from)}`);
  if (m.to?.length) H.push(`To: ${m.to.map(encodeAddress).join(', ')}`);
  if (m.cc?.length) H.push(`Cc: ${m.cc.map(encodeAddress).join(', ')}`);
  if (m.bcc?.length) H.push(`Bcc: ${m.bcc.map(encodeAddress).join(', ')}`);
  if (m.replyTo) H.push(`Reply-To: ${encodeAddress(m.replyTo)}`);
  ...
  for (const [k, v] of Object.entries(m.extraHeaders ?? {})) H.push(`${k}: ${v}`);
  ...
  const attachmentPart = (a: LoadedAttachment) =>
...
export const toBase64Url = (s: string | Buffer) => Buffer.from(s).toString('base64url');
...
function signatureRegex(text: string): RegExp {
  const escaped = text
    .trim()
    .split(/[\s\u00a0]+/)
    .map((w) => w.replace(/[.*+?^${}()|[\]\\]/g, '\\$&'))
...
export function stripSignature(text: string, sig?: Signature): string {
  const body = text.replace(/\r/g, '').replace(/\s+$/, '');
  if (!sig?.text) return body;
...
export function signText(text: string, sig?: Signature): string {
  const stripped = stripSignature(text, sig);
  if (!sig?.text) return stripped;
  const clean = sig.text.replace(/\u00a0/g, ' ').trim();
 ...
 */
export function htmlFromText(text: string, sig?: Signature): string {
  return signHtml(textToHtml(stripSignature(text, sig)), sig);
}

/** Append the HTML signature unless the body already carries one. */
export function signHtml(html: string, sig?: Signature): string {
  if (!sig?.html) return html;
  if (html.includes(SIG_MARKER)) return html;
  let body = html;
  if (sig.text)
...
export function signatureFromText(text: string): Signature {
  return { text: text.trim(), html: textToHtml(text.trim()) };
}

// --- Quoting for replies and forwards ---------------------------------------
...
function attributionLine(src: QuotedSource): string {
  const when = src.date ? formatQuoteDate(src.date) : 'an earlier date';
  return `On ${when}, ${src.from ?? 'someone'} wrote:`;
}

function formatQuoteDate(dateHeader: string): string {
...
export function quoteText(src: QuotedSource): string {
...
export function quoteHtml(src: QuotedSource): string {
  const inner = src.html ?? textToHtml(src.text ?? '');
  return (
    `<div class="gmail_quote"><div dir="ltr" class="gmail_attr">${escapeHtml(attributionLine(src))}<br></div>` +
    `<blockquote class="gmail_quote" style="margin:0px 0px 0px 0.8ex;border-left:1px solid rgb(204,204,204);padding-left:1ex">` +
    `${inner}</blockquote></div>`
  );
}

function forwardHeaderLines(src: QuotedSource): [string, string][] {
  const lines: [string, string][] = [];
  if (src.from) lines.push(['From', src.from]);
  if (src.date) lines.push(['Date', formatQuoteDate(src.date)]);
  if (src.subject) lines.push(['Subject', src.subject]);
  if (src.to?.length) lines.push(['To', src.to.join(', ')]);
  if (src.cc?.length) lines.push(['Cc', src.cc.join(', ')]);
  return lines;
}

export function forwardText(src: QuotedSource): string {
...
export function forwardHtml(src: QuotedSource): string {
  const head = forwardHeaderLines(src)
    .map(([k, v]) => `${escapeHtml(k)}: ${linkify(escapeHtml(v))}<br>`)
    .join('');
  const inner = src.html ?? textToHtml(src.text ?? '');
  return (
    `<div class="gmail_quote"><div dir="ltr" class="gmail_attr">---------- Forwarded message ---------<br>${head}</div><br><br>` +
    `${inner}</div>`
  );
}
...
export function withoutSelf(addrs: string[], self: Set<string>): string[] {
  const seen = new Set<string>();
  return addrs.filter((a) => {
    const b = bareAddress(a);
    if (self.has(b) || seen.has(b)) return false;
    seen.add(b);
    return true;
  });
}
