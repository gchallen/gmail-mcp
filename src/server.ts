...
export const SERVER_INSTRUCTIONS =
  'Gmail for one account. Read with search_threads / search_messages, then get_thread or get_message (default ' +
...
export function createServer(): McpServer {
  const server = new McpServer(
    { name: 'gmail-mcp', version: pkg.version },
    ...
    server.registerTool(
      ...
      async (args: Record<string, unknown>) => {
        try {
          ...
          return {
            content: [{ type: 'text' as const, text: `Error: ${explainError(e)}` }],
            isError: true,
          };
        }
      },
