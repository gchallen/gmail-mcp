import { z } from 'zod';
import { GmailService, LABEL_COLOR_PRESETS } from './gmail.js';
import { FORMAT_DESCRIPTION } from './format.js';

/**
 * Tool definitions, in one table so the MCP server and the `gmail-mcp call`
 * CLI share them. Names and parameter names follow the official Gmail MCP
 * server (gmailmcp.googleapis.com) where a tool exists there, so prompts and
 * skills written against it keep working; everything else is additive.
 */
export interface ToolDef<S extends z.ZodRawShape = z.ZodRawShape> {
  ...
  readOnly?: boolean;
  destructive?: boolean;
  handler: (args: z.infer<z.ZodObject<S>>, gmail: GmailService) => Promise<unknown>;
}
  ...
  .union([
    ...
    z.object({
      ...
      content: z
        ...
        .optional()
        .describe(
          'Base64 content, only when the file is not on disk. Prefer "path" for anything larger than a few KB.',
        ),
      filename: z
        .string()
        .optional()
        .describe(
          'Filename shown to the recipient. Defaults to the basename of "path". Required with "content".',
        ),
      mimeType: z
        .string()
        .optional()
      ...
      inline: z
        ...
        .optional()
        .describe(
          'Attach as an inline image referenced from htmlBody as <img src="cid:CONTENT_ID">.',
        ),
      contentId: z
        .string()
        .optional()
...
const attachments = z
  ...
  .optional()
  .describe(
    "Files to attach, as local paths (preferred: any size up to Gmail's 25 MB total) or base64 objects.",
  );

const QUERY_HELP =
  'Gmail search syntax, exactly as in the Gmail search box. Operators: from:, to:, cc:, bcc:, subject:, ' +
...
const BODY_HELP =
  'Plain-text body. One line per paragraph, no hard wrapping; Gmail wraps on display and links bare URLs. ' +
  'Markdown does NOT render: for bullets or other formatting supply htmlBody instead.';
const HTML_HELP =
  'HTML body, used when the message needs formatting such as <ul><li> lists. When given, "body" becomes the plain-text ' +
  'alternative (derived from the HTML if omitted). Do not include the signature; it is appended.';
const SIGNATURE_HELP =
  'Set false to send without the configured signature. By default the signature (from config.json, GMAIL_SIGNATURE, or ' +
...
export const tools: ToolDef<any>[] = [
  ...
  def({
    name: 'get_profile',
    description:
      'The authenticated account: email address, total message and thread counts, current history ID. Use it to learn whose mailbox this is.',
    schema: {},
    readOnly: true,
  ...
  def({
    name: 'search_threads',
    description:
      'Search conversations (threads). Returns one row per thread: subject, snippet, participants, message count, last date, ' +
    ...
    schema: {
      ...
      view: z
        ...
        .optional()
        .describe(
          '"minimal" (default): thread summary. "metadata": summary without subject or snippet. "messages": also list each message\'s headers.',
        ),
    },
    readOnly: true,
  ...
  def({
    ...
    description:
      'Search individual messages rather than threads: one row per message with headers, snippet and labels. Better than ' +
    ...
    schema: {
      ...
      format: z
        ...
        .optional()
        .describe(
          'Per-message detail. Default "minimal" (no bodies). "plain_text" fetches bodies too: slower and larger.',
        ),
    },
    readOnly: true,
  ...
  def({
    name: 'get_message',
    description:
      'Read one message by ID: headers, labels, attachment list and body. Not for drafts; use get_draft for those.',
  ...
  def({
    name: 'get_thread',
    description:
      'Read a whole conversation: every message in the thread, oldest first, with bodies. Drafts in the thread are included as messages with the DRAFT label.',
  ...
  def({
    ...
    description:
      'List the attachments on a message: filename, MIME type, size, attachment ID, and whether it is an inline image.',
  ...
  def({
    ...
    description:
      'Download attachments from a message to a local directory and return the written paths. All regular attachments by ' +
  ...
  def({
    name: 'create_draft',
    description:
      'Create a draft. With replyToMessageId it becomes a reply in that thread (subject, In-Reply-To and References set; ' +
  ...
  def({
    name: 'update_draft',
    description:
      'Edit an existing draft. Only the fields given change; recipients, subject, threading and attachments are kept ' +
  ...
  def({
    name: 'get_draft',
    description:
      'Read a draft by draft ID (not message ID): recipients, subject, body, attachments and a Gmail link.',
  ...
  def({
    name: 'list_drafts',
    description:
      'List drafts, newest first, with draft IDs. Filter with a Gmail query. view "full" adds bodies.',
    schema: {
      query: z.string().optional().describe(QUERY_HELP),
  ...
  def({
    name: 'delete_draft',
    description:
      'Discard a draft permanently by draft ID. Does not affect the thread it belongs to.',
  ...
  def({
    name: 'send_message',
    description:
      'Send a new message immediately. To thread it into an existing conversation give replyToMessageId (quotes and ' +
    ...
    handler: async (a, g) => {
      if (a.draftId) return g.sendDraft(a.draftId);
  ...
  def({
    name: 'reply',
    description:
      "Reply to a message with Gmail's default recipients: the sender (or Reply-To), plus everyone on To and Cc except " +
  ...
  def({
    name: 'forward',
    description:
      'Forward a message: your comment on top, then the standard "Forwarded message" block with the original headers ' +
  ...
  def({
    name: 'list_labels',
    description:
      'All labels with IDs, names, type (system or user), colors and visibility. System labels: INBOX, UNREAD, STARRED, IMPORTANT, SENT, DRAFT, TRASH, SPAM, CATEGORY_*. Label tools accept either an ID or an exact name.',
    schema: {},
    readOnly: true,
  ...
  def({
    name: 'create_label',
    description:
      'Create a label. Nested labels use "/" (Projects/Alpha); missing parents are created unless autoCreateParents is false. Color by preset name or by hex pair from Gmail\'s palette.',
    schema: {
      name: z.string(),
      colorPreset: z
        .string()
        .optional()
        .describe(
          `One of: ${Object.keys(LABEL_COLOR_PRESETS).join(', ')} (LABEL_COLOR_PRESET_ prefix accepted).`,
        ),
      backgroundColor: z
        .string()
        .optional()
        .describe(
          "Hex color from Gmail's label palette, e.g. #fb4c2f. Arbitrary colors are rejected by Gmail.",
        ),
  ...
  def({
    ...
    handler: async (a, g) =>
      g.updateLabel({ ...a, labelId: (await g.resolveLabelIds([a.labelId]))[0] }),
  }),
  def({
    name: 'delete_label',
    description:
      'Delete a user label. Messages keep their other labels. System labels cannot be deleted.',
    schema: { labelId: z.string().describe('Label ID or exact name.') },
    destructive: true,
  ...
  def({
    name: 'label_message',
    description:
      'Add labels to one message. IDs or exact names; system labels too (STARRED, IMPORTANT, INBOX, UNREAD). Use trash_message or mark_message_spam for those states.',
  ...
  def({
    ...
    description:
      'Remove labels from one message. Removing INBOX archives it; removing UNREAD marks it read.',
    schema: { messageId: z.string(), labelIds: z.array(z.string()).min(1) },
    handler: (a, g) => g.modifyMessage(a.messageId, [], a.labelIds),
  }),
  def({
    ...
    description:
      'Add and remove labels on one message in a single call, e.g. move between labels or mark read and archive at once.',
    schema: {
      messageId: z.string(),
  ...
  def({
    name: 'label_thread',
    description:
      'Add labels to every message in a thread (and, for user labels, future messages in it).',
  ...
  def({
    name: 'unlabel_thread',
    description:
      'Remove labels from every message in a thread. Removing INBOX archives the conversation; removing UNREAD marks it read.',
    schema: { threadId: z.string(), labelIds: z.array(z.string()).min(1) },
    handler: (a, g) => g.modifyThread(a.threadId, [], a.labelIds),
  }),
  def({
  ...
  def({
    ...
    description:
      'Bulk label change: over a list of message IDs, or over every message matching a Gmail query (up to maxMessages, ' +
    ...
    destructive: true,
    handler: (a, g) => {
      if (!a.messageIds?.length && !a.query) throw new Error('Give messageIds or a query.');
      if (!a.addLabelIds?.length && !a.removeLabelIds?.length)
        throw new Error('Give addLabelIds and/or removeLabelIds.');
  ...
  def({
    name: 'trash_message',
    description:
      "Move one message to Trash (also works on a draft's message ID). Prefer trash_thread for whole conversations.",
    schema: { messageId: z.string() },
    destructive: true,
  ...
  def({
    ...
    description:
      'Compatibility alias for the official server: TRASH or SPAM one message. Prefer trash_message / mark_message_spam.',
    schema: { messageId: z.string(), labelOption: z.enum(['TRASH', 'SPAM']) },
    destructive: true,
    handler: (a, g) =>
      a.labelOption === 'TRASH' ? g.trashMessage(a.messageId) : g.spamMessage(a.messageId),
  }),
  def({
    name: 'apply_sensitive_thread_label',
    description:
      'Compatibility alias for the official server: TRASH or SPAM a whole thread. Prefer trash_thread / mark_thread_spam.',
    schema: { threadId: z.string(), labelOption: z.enum(['TRASH', 'SPAM']) },
    destructive: true,
    handler: (a, g) =>
      a.labelOption === 'TRASH' ? g.trashThread(a.threadId) : g.spamThread(a.threadId),
  }),
];
